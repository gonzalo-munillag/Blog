...
[Jump to teh replicated figure if you are impatient](#results) :) 
...
1. b. IMPORTANT: The authors use an approximation (5.1 equation 4-5) based on the release dataset and its neighbors (delta_v) and sensitvity based on unbounded DP (delta_f) in order to calculate the upper bound. In this notebook however, I use the definition for sensitvity based on bounded DP as delta_v, because to the best of my knowledge, it should be equal to the one the authors chose as long as I keep the hamming distance to 1. Nonetheless, the results obtained are the same as the author's. Perhaps back then, the terms bounded and unbounded DP were not used.
